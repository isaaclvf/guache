...
int leftMostPoint(Polygon *polygon)
{
  int min = 0;
  for (int i = 1; i < polygon->vertexCount; i++)
  {
    if (polygon->vertices[i][0] < polygon->vertices[min][0])
    {
      min = i;
    }
  }
...
// Função que verifica a orientação de três pontos (vetor cruzado)
int orientation(Point p, Point q, Point r)
{
  float val = (q.y - p.y) * (r.x - q.x) - (q.x - p.x) * (r.y - q.y);
  if (val == 0)
    return 0;               // colinear
  return (val > 0) ? 1 : 2; // 1: sentido horário, 2: sentido anti-horário
}

// Algoritmo de Marcha de Jarvis
void jarvisMarch(Polygon *polygon)
{
  printf("Entrou na função jarvisMarch\n");

  if (polygon->vertexCount < 3)
    return;

  int l = leftMostPoint(polygon);
  int p = l, q;

  printf("Ponto mais a esquerda: (%.2f, %.2f)\n", polygon->vertices[l][0], polygon->vertices[l][1]);

  float hull[MAX_VERTICES][2];
  int hullSize = 0;

  do
  {
    hull[hullSize][0] = polygon->vertices[p][0]; // Adiciona o ponto ao fecho convexo
    hull[hullSize][1] = polygon->vertices[p][1];
    hullSize++;
    ...
    for (int i = 0; i < polygon->vertexCount; i++)
    {
      Point p_point = {.x = polygon->vertices[p][0], .y = polygon->vertices[p][1]};
      Point i_point = {.x = polygon->vertices[i][0], .y = polygon->vertices[i][1]};
      Point q_point = {.x = polygon->vertices[q][0], .y = polygon->vertices[q][1]};

      if (orientation(p_point, i_point, q_point) == 2)
      {
        q = i;
      }
    }
  ...
  } while (p != l); // Continua até voltar ao ponto inicial

  printf("Terminou de achar o fecho convexo\n");
  for (int i = 0; i < hullSize; i++)
  {
    printf("\t(%.2f, %.2f)\n", hull[i][0], hull[i][1]);
  }

  // Substitui vértices do polígono pelos pontos do fecho
  polygon->vertexCount = hullSize;

  printf("Substituindo os vértices do polígono\n");
  for (int i = 0; i < hullSize; i++)
  {
    polygon->vertices[i][0] = hull[i][0];
    polygon->vertices[i][1] = hull[i][1];
  }
...
void keyPress(unsigned char key, int x, int y)
{
  ...
  else if (key == 'f')
  {
    ...
    {
      printf("Apertou f\n");
      jarvisMarch(&selectedPolygon->polygon);
    }
  }
  else if (key != 'z')
  {
